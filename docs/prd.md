...
10. **NFR10:** The Anchor programs MUST be deployed to the Solana Devnet for the
    POC MVP.
11. **NFR11:** Program testing SHALL be conducted using `vitest` with `@solana/kit`
    and `solana-kite` for a modern, type-safe testing experience. Due to
    limitations in time manipulation, comprehensive testing of time-based logic
    (e.g., precise check-in windows) will be deferred to a future phase.
12. **NFR12:** Final validation SHALL be performed using the `vitest`
    test suite on Devnet for a real network environment.
...
- **Mint Permit Mechanism:** Upon successful completion of the commitment
  (streak target met and withdrawal initiated), the `srs_vault` program will perform
...
writing extensive tests using modern Solana tooling and creating clear, comprehensive
documentation.
