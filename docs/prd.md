...
10. **FR10:** The system SHALL close the vault account upon successful
    withdrawal.
11. **FR11:** Upon successful withdrawal and closure of the vault account, the
...
12. **FR12:** The system SHALL provide a separate, permissionless program
    (`nft-minter`) for minting a unique NFT credential.
13. **FR13:** The `nft-minter` program SHALL require a valid "Mint Permit" from
    the `vault` program to proceed with minting.
14. **FR14:** The `nft-minter` program SHALL consume and close the "Mint Permit"
    upon successful NFT creation.
15. **FR15:** The "Mint Permit" SHOULD contain metadata linking it to the
    specific commitment, such as the `deck_id` associated with the vault.
16. **FR16:** The minted NFT SHOULD include metadata reflecting the commitment
    details (e.g., streak length, completion date).
17. **FR17:** The system SHALL handle errors gracefully, particularly if the CPI
...
- **Mint Permit Mechanism:** Upon successful completion of the commitment
  (streak target met and withdrawal initiated), the `srs-vault` program will
