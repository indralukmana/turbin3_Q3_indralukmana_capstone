...
11. **NFR11:** Program testing SHALL be conducted using `vitest` with
    ...
    Due to limitations in time manipulation, comprehensive testing of time-based
...
- **Integration Tests:** For testing the interaction between program
  instructions, account state changes, and the flow between the `srs_vault` and
...
- **PDA Structure:** The `srs_vault` program will manage a PDA for each
  ...
  PDA generated by the `srs_vault`.
- **Mint Permit Mechanism:** Upon successful completion of the commitment
  (streak target met and withdrawal initiated), the `srs_vault` program will
  perform a Cross-Program Invocation (CPI) to the `nft-minter` program to create
  a single-use `MintPermit` PDA. This `MintPermit` serves as proof of successful
...
- **AC 1.1.2:** The program SHALL have an `initialize_vault` instruction that
  takes `initial_deposit_amount` and `streak_target` (and implicitly, a
  `deck_id`) as parameters.
- **AC 1.1.3:** The `initialize_vault` instruction SHALL initialize a new
  `Vault` PDA account, seeded by the user's public key and the `deck_id`.
- **AC 1.1.4:** The `initialize_vault` instruction SHALL transfer the specified
  `initial_deposit_amount` of SOL from the user's wallet to the newly created
  `Vault` PDA.
- **AC 1.1.5:** The `initialize_vault` instruction SHALL correctly initialize
  the `Vault` account fields: `user` to the signer, `initial_deposit_amount`,
...
- **AC 1.4.6:** The `mint_credential` instruction SHALL use the Metaplex library
  to mint a new NFT to the user's wallet. The NFT metadata SHALL reflect
  commitment details (e.g., `deck_id`, streak length, completion date).
...
writing extensive tests using modern Solana tooling and creating clear,
...
- **AC 2.1.1:** Unit tests SHALL cover the `initialize_vault` instruction,
  verifying account initialization, SOL transfer, and state setup under normal
  conditions.
...
- **AC 2.2.1:** Integration tests SHALL simulate the full user journey:
  `initialize_vault` -> `check_in`(s) -> `withdraw`, verifying core state
  transitions. Note: Precise time simulation is deferred.
...
- **AC 2.3.2:** Documentation SHALL describe each instruction
  (`initialize_vault`, `check_in`, `withdraw`, `mint_credential`) available in
  the programs, including parameters and expected outcomes.
...
- **AC 2.3.4:** Documentation SHALL provide clear instructions for building,
  testing (using modern Solana tooling, with a note about deferred time-based
  testing), and deploying the programs to Devnet.
